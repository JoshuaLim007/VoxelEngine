...
#define EPSILON 0.000000001f
	...
	extern __device__ __constant__ float gradMap[16][3];
	...
	__device__ __forceinline__ float simplexNoise(float3 pos, float scale, int seed)
	{
	...
	// Fast function for fBm using perlin noise
	__device__ __forceinline__ float repeaterPerlinBounded(float3 pos, float scale, int seed, int n, float lacunarity, float decay, float threshold)
	{
		float acc = 1.0f;
		float amp = 1.0f;
	...
	__device__ __forceinline__ float repeaterSimplexAbs(float3 pos, float scale, int seed, int n, float lacunarity, float decay)
	{
		float acc = 0.0f;
		float amp = 1.0f;
	...
	__device__ __forceinline__ float repeaterSimplexBounded(float3 pos, float scale, int seed, int n, float lacunarity, float decay, float threshold)
	{
		float acc = 1.0f;
		float amp = 1.0f;
