...
namespace GPUDDA {
	...
	private:
		...
		VoxelRaytracer3D& operator=(const VoxelRaytracer3D&) = delete;
		VoxelRaytracer3D(VoxelRaytracer3D&&) = delete;
		VoxelRaytracer3D& operator=(VoxelRaytracer3D&&) = delete;
		VoxelRaytracer3D() = default;
	...
	public:
		VoxelRaytracer3D(size_t count) {
			...
			d_results = nullptr;
			cudaMalloc((void**)&d_results, sizeof(float3) * count);
			d_results_normal = nullptr;
			cudaMalloc((void**)&d_results_normal, sizeof(float3) * count);
			d_results_steps = nullptr;
			cudaMalloc((void**)&d_results_steps, sizeof(int) * count);
			d_origins = nullptr;
			d_rays = nullptr;
			cudaMalloc((void**)&d_origins, sizeof(float3) * count);
			cudaMalloc((void**)&d_rays, sizeof(float3) * count);
		}
		~VoxelRaytracer3D() {
			Free();
		}

		GPUDDA::VoxelBuffer3D* GetVoxelBuffer() {
			return gpu_VoxelBuffer;
		}
		GPUDDA::VoxelBuffer3D* GetVoxelBufferDatas() {
	...
	static std::tuple<VoxelBuffer3D, VoxelBuffer3D*, Bounds3Df*> GenerateLowresVoxelBuffer(const VoxelBuffer3D& originalData, int factor = 4) {
		...
		struct ThreadParams {
			...
			VoxelBuffer3D* low_res_grid_data = nullptr;
			Bounds3Df* low_res_per_chunk_bounds = nullptr;
			const VoxelBuffer3D* high_res_buffer = nullptr;
			bool* low_res_grid_contains_voxels = nullptr;
		};

		auto HandleThread = [](ThreadParams params) {
		...
		size_t low_res_cols = originalData.dimensions[0] / (size_t)factor;
		size_t low_res_rows = originalData.dimensions[1] / (size_t)factor;
		size_t low_res_slices = originalData.dimensions[2] / (size_t)factor;
		...
		for (int i = 0; i < threads_count; i++) {
			...
			params.high_res_buffer = &originalData;
