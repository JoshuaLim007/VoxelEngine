...
#endif
void DDARayTraversal(VoxelBuffer<2> VoxelBuffer, float2 start, float2 direction, float4 *bounds, int max_steps,
                     float4 *per_voxel_bounds, int per_voxel_bounds_scale,

                     // return values
                     bool *out_hit, bool *out_isOutOfBounds, float2 *out_HitCell, float2 *out_HitIntersectedPoint,
                     int *out_stepsTaken)
{
    float x = start.x;
    float y = start.y;
            ...
            *out_HitCell = make_float2(cell_x, cell_y);
            if (per_voxel_bounds)
            {
                ...
                if (grid[cell_y * cols + cell_x] == 1 && bmin_x <= bmax_x)
                {
                    ...
                    if (RayIntersectsAABB(make_float2(temp_x, temp_y), direction, make_float2(bmin_x, bmin_y),
                                          make_float2(bmax_x, bmax_y), nullptr, nullptr))
                    {
                        *out_hit = true;
                        break;
                    }
...
float2 Raytrace(float2 origin, float2 ray, VoxelBuffer<2> chunks, VoxelBuffer<2> *chunksData,
                float4 *chunkBoundingBoxes, int factor)
{
    ...
    {
        float2 intersect;
        if (RayIntersectsAABB(start, direction, make_float2(0, 0),
                              make_float2(chunks.dimensions[1], chunks.dimensions[0]), &intersect, nullptr))
        {
    ...
    while (true)
    {
...
#endif
        DDARayTraversal(chunks, start, direction, nullptr, 100, chunkBoundingBoxes, factor, &hit, &isOutOfBounds,
                        &hitCell, &hitIntersectedPoint, &stepsTaken);
#ifdef RECORD_INTERSECTED_POINTS
        ...
        if (hit && !isOutOfBounds)
        {
            ...
            DDARayTraversal(chunkData, start_high_res, direction, &chunkBounds, 100, nullptr, 0, &hit1, &isOutOfBounds1,
                            &hitCell1, &hitIntersectedPoint1, &stepsTaken1);
            ...
            if (!hit1)
            {
                start = make_float2(hitIntersectedPoint1.x + eps * direction.x + hitCell.x * factor,
...
bool initSDL(SDL_Window **window, SDL_Renderer **renderer, int width, int height)
{
    ...
    *window = SDL_CreateWindow("SDL Window", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, width, height,
                               SDL_WINDOW_SHOWN);
    if (*window == nullptr)
    {
...
int main()
{
    ...
    raytracer.UploadVoxelBufferDataBounds(reinterpret_cast<Bounds<float2> *>(low_res_chunk_bounds),
                                          low_res_buffer.dimensions[0] * low_res_buffer.dimensions[1]);
    ...
    while (!quit)
    {
        while (SDL_PollEvent(&e) != 0)
        {
            ...
            if (setRayDirection)
            {
                int mouseX, mouseY;
                SDL_GetMouseState(&mouseX, &mouseY);
                auto dir = make_float2((float)(mouseX - offsetX) / scale - origin.x,
                                       (float)(mouseY - offsetY) / scale - origin.y);
                ray = dir;
            }
        }
        ...
        {
            if (!gpuHitPos.valid[i])
                continue;                                             // Skip if no hit
            SDL_SetRenderDrawColor(renderer, 0x00, 0x00, 0xFF, 0xFF); // Blue color
            SDL_RenderDrawLine(renderer, origins[i].x * scale + offsetX, origins[i].y * scale + offsetY,
            ...
            SDL_RenderDrawLine(renderer, hit.x * scale + offsetX, hit.y * scale + offsetY,
        ...
        for (size_t i = 0; i < rays; i += is)
        {
            ...
            for (int w = 0; w < radius * 2; w++)
            {
                for (int h = 0; h < radius * 2; h++)
                {
                    ...
                    if ((dx * dx + dy * dy) <= (radius * radius))
                    {
                        SDL_RenderDrawPoint(renderer, gpuHitPos.hitPoint[i].x * scale + dx + offsetX,
                                            gpuHitPos.hitPoint[i].y * scale + dy + offsetY);
                    }
                }
            }
        }
